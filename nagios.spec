...
- Stop refetching objectjson.cgi's per-host/service notes_url/
  action_url/icon_image and group membership on every 90s SPA
  auto-refresh; that data only changes on a config reload, unlike
  status. Fetched once per page visit now. Part of addressing a
  real-install report of the Services page taking 5-10s to load on a
  large deployment. (A second, larger fix -- a genuine O(hosts *
  services) algorithmic bug in objectjson.cgi/statusjson.cgi's own
  servicelist query -- is on the perf/servicelist-quadratic-scan
  branch pending review before it's released.)
...
- Port extinfo.cgi's Comments (type=3) and Downtime (type=6) views
  onto the SPA, including a Delete quick action for each row, backed
  entirely by the existing statusjson.cgi commentlist/downtimelist
  queries (no new server-side code)
...
- Fix the SPA shell's #nav/#dashboard divs rendering stacked instead
  of side-by-side (the old frameset's 180px-sidebar layout was never
  replaced with real CSS when the frameset was removed). Reported
  from a real install running kuriyama19
...
- Remove dead WAP/WML output branches from cmd.cgi (unreachable since
  statuswml.cgi was removed in kuriyama2)
...
- Add an in-SPA all-services detail view with sort/refresh/servicegroup
  filter (status.cgi Phase C-1), mirroring the kuriyama4-6 hosts view
...
- Add sort (click column header) and 90s auto-refresh to the all-hosts
  detail view added in kuriyama4
